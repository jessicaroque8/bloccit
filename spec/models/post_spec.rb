...
RSpec.describe Post, type: :model do
   ...
   describe "after_create" do
      ...
      it "creates favorite on the post by the post creator" do
        expect(user.favorites.find_by_post_id(@new_post.id)).not_to be_nil
        @new_post.save!
      end

      it "sends an email to the post creator that they've favorited their post and will receive updates " do
        expect(FavoriteMailer).to receive(:new_post).with(@new_post).and_return(double(deliver_now: true))
        @new_post.save!
      end
   end
